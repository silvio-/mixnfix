...
public class CorrecaoUI {
    ...
    public CorrecaoUI(ModelEntradaProvaCorrecao model) throws IOException, SQLException  {
        _model = model;
        ...
        g.inicializarQuesitos(_model.getEntradaProvaCorrecao().getTipo());
        ...
        // --------------------------------------------------------------------
        // a partir dos pontos de controle definir o centro das células e o
        ...
        for (Quesito q : quesitos) {
            if (q.getTipo() == Quesito.TIPO_ALTERNATIVAS || q.getTipo() == Quesito.TIPO_SUBJETIVA_5) {
                ...
                if (!l.isEmpty()) {
                    ...
                    OptionField ft = (OptionField) _cellMapVariavel.getField(""+i+".0");
                    OptionField fu = (OptionField) _cellMapVariavel.getField(""+i+".1");
                    ft.getCell(tens).setSelected(true);
                    fu.getCell(units).setSelected(true);
                }
            }
    ...
    public String getDetalhamentoNota() {
        ...
        _prova.avaliarNota();
        _prova.detalharNota(detalhamento);
    ...
    public class PanelVisualizacaoProva extends JPanel {
        ...
        public BufferedImage newImage() {
            int phase = _model.getEntradaProvaCorrecao().getPhase();
        ...
        public PanelVisualizacaoProva() throws IOException {
            super();
            _image = ImageIO.read(new File(App.getConfiguracao().getProperty(ConfiguracaoMIXnFIX.datadir)+"/"+_model.getEntradaProvaCorrecao().getFoto()));
        }
        ...
        public void drawHardWork(Graphics g) {
            ...
            if (_model.getEntradaProvaCorrecao().getPhase() == 1) {
                g2.transform(new AffineTransform(0,-1,1,0,0,_image.getWidth()));
            }
            else if (_model.getEntradaProvaCorrecao().getPhase() == 3) {
